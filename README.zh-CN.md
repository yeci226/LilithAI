...
LilithAI 将 AI 对话接入游戏原本的互动菜单，保留最近的聊天记录，也能让回复带动莉莉丝的表情与原生操作，例如换睡衣、设置计时器／闹钟／番茄钟、播放音乐、换帽子眼镜、安静召回及坐卧睡醒。支持 OpenAI、Anthropic、Gemini、xAI、DeepSeek、Mistral、OpenRouter，以及本地运行的 Ollama、LM Studio。
...
在 [最新 Release](../../releases/latest) 下载纯文字包；需要语音时，再加入可重复使用的 [v0.10.20 语音附加包](../../releases/tag/v0.10.20)：

| 语音版本 | 需要下载 |
| --- | --- |
| 无语音 | 最新的 `text.zip` |
| 中文 | 最新的 `text.zip`＋`v0.10.20-voice-chinese.zip` |
| 日语 | 最新的 `text.zip`＋全部 `v0.10.20-voice-japanese-*.zip` |
| 中文＋日语 | 最新的 `text.zip`＋上述全部语音附加包 |

所有 ZIP 都直接解压到 `Lilith.exe` 所在文件夹；日语包因 GitHub 单文件 2 GiB 上限拆成多个文件。纯文字包已包含所需的 BepInEx，语音附加包只在语音内容更新时重新发布。
